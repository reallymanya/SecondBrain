...
const Modal = (props) => {
    ...
    return (<div className="fixed inset-0 h-screen w-full flex justify-center items-center z-50 px-4">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} ref={modalRef} onClick={props.onClick} className="absolute inset-0 bg-black/80 backdrop-blur-sm"/>
        ...
        <div className="p-6 space-y-6">
          <div className="space-y-4">
            <div>
              ...
              <Input ref={titleRef} placeholder="e.g. The Future of AI" maxLength={50} className="bg-white/[0.03] border-white/10"/>
            </div>

            <div>
              <label className="text-xs font-medium text-slate-400 mb-1.5 block uppercase tracking-wider">URL</label>
              <Input ref={linkRef} placeholder="https://..." className="bg-white/[0.03] border-white/10"/>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
              {categories.map((cat) => (<button key={cat.id} onClick={() => setCategory(cat.id)} className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all ${category === cat.id
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {mapTags.map((t) => (<button key={t} onClick={() => setTag(t)} className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all flex items-center gap-1.5 ${tag === t
