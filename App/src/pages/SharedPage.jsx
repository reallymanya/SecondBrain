    ...
    return (<div className="bg-[#030014] w-full min-h-screen text-white font-sans selection:bg-purple-500/30">
      ...
      <div className="p-8">
        {loading ? (<div className="flex justify-center py-20">
            ...
            {sharedData.length > 0 ? (sharedData.map((item, idx) => (<Card key={idx} type={item.contentType || "link"} tags={[item.tag]} title={item.title} link={item.link} date={item.date || new Date().toISOString()} onDelete={() => { }} // Read-only view
